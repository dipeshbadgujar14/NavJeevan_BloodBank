...
def Auth_User(request):
	...
	print("Auth_User", email, password, user_type)
	if user_type=="donor":
		user_list=UserDetails.objects.all()
		...
		for bank in bank_list :
			if bank.email==email and bank.password==password :
				...
				return HttpResponseRedirect('/') #Redirect To Blood Bank Profile
			else:
				error_msg="E-mail or Password is Incorrect"
	...
	if request.method=='POST':
		if request.POST.get('search_by') and request.POST.get('blood') and request.POST.get('country') and request.POST.get('state') and request.POST.get('city'):
			...
			if search_by=='donor':
				...
				return render(request, 'list.html', { 'output_list' : output_list })
				...
				return render(request, 'bank_list.html',{ 'output_bank_list' : output_bank_list , 'output_blood_list' : output_blood_list })					
